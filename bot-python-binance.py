...
class BinanceBot:
    ...
    def fetch_data(self, limit=100):
        """ดึงข้อมูลแท่งเทียน"""
        try:
            klines = self.client.get_klines(symbol=self.config['symbol'], interval=self.config['timeframe'], limit=limit)
    ...
    def place_order(self, order_type):
        ...
        try:
            ...
            if order_type == "BUY":
                order = self.client.order_market_buy(symbol=self.config['symbol'], quantity=amount)
            else:  # SELL
                order = self.client.order_market_sell(symbol=self.config['symbol'], quantity=amount)

            # เพิ่มจำนวน orders
            self.order_counts[order_type] += 1
            print(f"✅ คำสั่งสำเร็จ: {order['orderId']}")
